...
class UsersController extends Controller
{
  protected function companyUsers()
  {
    ...
    foreach ($companies as $company) {
      if ($company->users()->get()->count() > 0) {
        $company_users[$company->id] = $this->prepareUsersForReact(
          $company->users()->get()
        );
      } else {
  ...
  protected function prepareUsersForReact($users)
  {
    ...
    foreach ($users as $user) {
      ...
      foreach ($rights as $right) {
        switch ($right->type_of_right) {
          case 1:
            $users_new[$index]['rights'][$index_rights]['right_type'] =
        ...
        $users_new[$index]['rights'][$index_rights]['right_wight'] =
          $right->weight * $right->number_of_share;
        $users_new[$index]['rights'][$index_rights]['right_name'] =
          $right->name;

        $users_new[$index]['rights'][$index_rights]['right_grounds'] =
          $right->grounds;
        $index_rights++;
      }
      ...
      $users_new[$index]['permissions'] = $user->isAdmin()
        ? 'Администратор'
        : '';

      if (empty($users_new[$index]['permissions'])) {
        $users_new[$index]['permissions'] .= $user->isVote()
      ...
      if (empty($users_new[$index]['permissions'])) {
        $users_new[$index]['permissions'] .= $user->isGovernance()
          ? 'Член правления'
          : '';
      } else {
        $users_new[$index]['permissions'] .= $user->isGovernance()
      ...
      if (empty($users_new[$index]['permissions'])) {
        $users_new[$index]['permissions'] .= $user->isManageItems()
          ? 'Модератор'
          : '';
      } else {
        $users_new[$index]['permissions'] .= $user->isManageItems()
      ...
      if (empty($users_new[$index]['permissions'])) {
        $users_new[$index]['permissions'] .= $user->isAccess()
          ? 'Допущен к сайту'
          : '';
      } else {
        $users_new[$index]['permissions'] .= $user->isAccess()
      ...
      if (empty($users_new[$index]['permissions'])) {
        $users_new[$index]['permissions'] .= $user->isSuperAdmin()
          ? 'Супер Администратор'
          : '';
      } else {
        $users_new[$index]['permissions'] .= $user->isSuperAdmin()
    ...
    foreach ($users_new as $_) {
      $users_new[$index]['permissions'] = explode(
        '=',
        $users_new[$index]['permissions']
      );
      $index++;
  ...
  public function submitUpdateProfile(Request $request)
  {
    ...
    $user_additionals = UsersAdditionalFields::updateOrCreate(
      ...
      [
        ...
        'ownership' => isset($parameters['ownership'])
    ...
    $user->update([
      ...
      'additional_id' => isset($user_additionals)
        ? $user_additionals->id
        : null,
    ]);
  ...
  public function addOrUpdateForm(Request $request)
  {
    ...
    foreach ($permissions as $permission) {
      switch ($permission) {
        ...
        case $permission == Permission::VOTE:
          array_push($permissions_name, [
            Permission::VOTE => 'Допущен к голосованию',
          ]);
          break;
        case $permission == Permission::ACCESS:
          array_push($permissions_name, [
            Permission::ACCESS => 'Доступ к сайту',
          ]);
          break;
        case $permission == Permission::MANAGE_ITEMS:
          array_push($permissions_name, [
            Permission::MANAGE_ITEMS => 'Модератор',
          ]);
          break;
        case $permission == Permission::GOVERNANCE:
          array_push($permissions_name, [
            Permission::GOVERNANCE => 'Член Правления',
          ]);
          break;
      }
    }
    ...
    return view('users.addOrUpdate', [
      ...
      'update' => isset($request->user_update)
        ? User::find($request->user_update)
        : false,
      'positions' => $positions,
      'companies' => Company::all(),
    ]);
  ...
  public function addOrUpdate(Request $request)
  {
    ...
    if ($validator->fails()) {
      return redirect('manage/add')->withErrors($validator)->withInput();
    }
      ...
      if (isset($parameters['position'])) {
        ...
        foreach ($users as $user) {
          if ($user->position_id == $parameters['position']) {
            ...
            unset(
              $arr_user_permitions[
                array_search(Permission::GOVERNANCE, $arr_user_permitions)
              ]
            );
    ...
    if (isset($parameters['job']) || isset($parameters['ownership'])) {
      $user_additionals = UsersAdditionalFields::updateOrCreate(
        [
          'id' => isset($request->id)
            ? User::find($request->id)->additional_id
            : '',
        ],
        [
          'job' => isset($parameters['job']) ? $parameters['job'] : '',
      ...
      if (isset($parameters['companies'])) {
        ...
        foreach ($parameters['companies'] as $company_id) {
          Company::find($company_id)
            ->users()
            ->attach([$user->id => ['company_id' => $company_id]]);
        }
      }
    }
      ...
      if ($count_value > 1) {
        ...
        if (isset($user->position_id)) {
          $error =
            'Пользователь ' .
            User::find($key)->name .
            ' не может занимать должность ' .
            Position::find($count_value)->position .
            ', т.к. он уже занимает должность ' .
            Position::find(User::find($key)->position_id)->position .
            '!';
        } else {
          $error =
            'Пользователь ' .
            User::find($key)->name .
            ' не может занимать несколько должностей сразу!';
        }
      }
    }
    if (!$error) {
      foreach ($inputs as $position_id => $user_id) {
        if (is_numeric($position_id)) {
          //dd($user_id);
          //$users = User::all();
          $users = Company::find(session('current_company')->id)
            ->users()
            ->get();
          foreach ($users as $user) {
  ...
  public function getAllUsersJson()
  {
    ...
    $users = $currentCompany->users()->get();
