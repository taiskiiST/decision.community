...
class Question extends Model
{
  ...
  public function answerThatUserVote(User $user)
  {
    ...
    foreach ($answers->get() as $answer) {
      if (
        Vote::where('answer_id', '=', $answer->id)
  ...
  public function middleAnswerThatAllUsersMarkOnReport()
  {
    ...
    $count_users_that_voted_by_question = Vote::where(
      'question_id',
      '=',
      $this->id
    )->count();
  ...
  public function countWeightVotesByQuestion(int $typeOfRight)
  {
    $weights = 0;
    $votes = Vote::where('question_id', '=', $this->id)->get();
  ...
  public function countQuestionsAllAnonymous(Question $question)
  {
    ...
    $answers = $question->answers;
    foreach ($answers as $answer) {
      $votes[$answer->id] = AnonymousVote::where(
        'answer_id',
        '=',
        $answer->id
      )->count();
