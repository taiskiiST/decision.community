        ...
        <table className="min-w-full divide-y divide-gray-200 ">
          <thead className="bg-gray-50">
            <tr>
              ...
              {poll_finished && (
                <th scope="col" className="relative px-6 py-3">
                  <span className="text-green-600">
                    Голосование окончено {poll_full.finished}
                  </span>
                </th>
              )}
              {!poll_finished && poll_start && (
                <th scope="col" className="relative px-6 py-3">
                  <span className="text-green-600">Голосование началось</span>
                </th>
              )}
              {!poll_finished && !poll_start && (
                <th scope="col" className="relative px-6 py-3">
              ...
              {is_admin && (
                <th scope="col" className="relative px-6 py-3">
                  <a
                    href={`/polls/${poll_full.id}/requisites`}
                    className="submit-button inline-flex items-center rounded-md border
                                       border-transparent bg-indigo-600 px-4 py-2 text-sm
          ...
          <tbody>
            {questions.map((question, index) => (
              <tr
                className={`bg-white ${index % 2 === 0 ? 'bg-gray-200' : ''}`}
                key={index}
              >
