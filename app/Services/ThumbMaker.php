...
class ThumbMaker
{
    ...
    public function makeFromImageUrl(string $imageUrl, string $outputPath): bool
    {
        ...
        if ($response->failed()) {
            logger(__METHOD__ . ' - request failed, client error: ' . $response->clientError() . ', server error: ' . $response->serverError());
        ...
        $originalImageSavedSuccessfully = Storage::put($originalImagePath, $response->body());
     ...
     * @return bool
     */
    public function makeFromFile(string $filePath, string $outputFilePath, bool $removeOriginalFile = false): bool
    {
        ...
        if ($returnVar !== 0) {
            logger(__METHOD__ . " - error occurred, filePath: {$filePath}, output path: {$outputFilePath}, return var: {$returnVar}, output:", $output);
        }
