...
class Question extends React.Component {
  ...
  checkFileTypeSizeName(type, uploadFile, id) {
    ...
    if (
      (!this.checkAllUploadsFilesSize || !this.checkUploadFileSize(newFile)) &&
      this.checkAllUploadsFilesName &&
      this.checkUploadFileName(newFile)
    ) {
      this.setState({ isValidAllUploadFiles: false });
  ...
  onEditorStateChange = (editorStateText) => {
    ...
    if (question['id']) {
      document.getElementById('question_text_' + question['id']).value = json;
    } else {
                ...
                <div className="mt-5 md:col-span-2 md:mt-0">
                  <div className="col-span-2 inline-flex">
                    <div className="px-4 sm:px-0">
                      ...
                      {question && (
                        <h3 className="ml-6 mt-6 text-lg font-medium leading-6 text-gray-900">
                          Изменения вопроса {current_num_question} по теме:{' '}
                          {poll['name']} с id {question['id']}
                        </h3>
                      )}
                    </div>
                    {isSuggestedQuestion && (
                      <div className="flex items-center bg-white px-4 py-5 sm:p-6">
                        <div className="flex items-center">
                          ...
                          >
                            Окончить режим редактирования (PS Если вы ставите
                            галочку, то после подтверждения вопрос будет
                            доступен к голосованию всем пользователям и изменить
                            вопрос будет невозможно! В случае ошибки, вопрос
                            можно будет только удалить.)
                          </label>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="inline-flex w-full flex-row">
                      ...
                      {question && (
                        <label
                          ...
                          className="block text-sm font-medium text-gray-700"
                        >
                          Введите текст вопроса №{current_num_question}{' '}
                          (поддерживается HTML формат){' '}
                        </label>
                      )}
                    </div>
                          ...
                          toolbar={{
                            ...
                            image: {
                              ...
                              uploadCallback: async (file) => {
                                return toBase64(file).then((fileInBase64) => ({
                                  data: {
                                    link: fileInBase64,
                                  },
                                }));
