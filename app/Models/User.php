...
class User extends Authenticatable
{
  ...
  {
    return $this->isAdmin() ||
      in_array(Permission::MANAGE_ITEMS, explode(',', $this->permissions));
  }
  ...
  {
    return $this->isAdmin() ||
      in_array(Permission::GOVERNANCE, explode(',', $this->permissions));
  }
  ...
  {
    return $this->isAdmin() ||
      in_array(Permission::VOTE, explode(',', $this->permissions));
  }
  ...
  public function position()
  {
    if (isset($this->hasOne(Position::class, 'id', 'position_id')->get()[0])) {
      return $this->hasOne(Position::class, 'id', 'position_id')->get()[0]
        ->position;
    } else {
      return '';
    }
  ...
  public function availableItems(
    ...
    return Item::when(is_null($parentId), function (Builder $builder) {
      return $builder->firstLevel();
    })->when($parentId >= 0, function (Builder $builder) use (
      $includeParent,
      $parentId
    ) {
    ...
    DB::transaction(function () use ($question, $answer, &$out) {
      $question->poll->update([
        'potential_voters_number' => $question->poll->isGovernanceMeeting()
          ? $question->poll->company->potentialWeightVotersNumberGovernance(
            TypeOfRight::UPON_OWNERSHIP
          ) //$question->poll->company->potentialVotersNumberGovernance()
          : $question->poll->company->potentialWeightVotersNumber(
            TypeOfRight::UPON_OWNERSHIP
          ), //$question->poll->company->potentialVotersNumber(),
      ]);
