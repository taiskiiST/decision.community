...
class Question extends React.Component {
  ...
  validateField(fieldName, value, pattern) {
    ...
    if (result_of_match) {
      if (result_of_match.groups['question_text']) {
        ...
        fieldValidationErrors.inputTextOfQuestion = inputTextOfQuestionValid
          ? ''
          : 'Поле текста вопроса обязательно для заполнения!';
      }
      if (result_of_match.groups['file_text']) {
        inputFilesTextValid = value.length > 0;
        fieldValidationErrors.inputFilesText = inputFilesTextValid
          ? ''
          : 'Поле текста описания файла обязательно для заполнения!';
      }
      if (result_of_match.groups['answer_text']) {
        inputAnswersValid['text_answer_' + this.state.answer.answer_id] =
          value.length > 0;
        fieldValidationErrors.inputAnswersValid[
          'text_answer_' + this.state.answer.answer_id
        ] = inputAnswersValid['text_answer_' + this.state.answer.answer_id]
          ? ''
          : 'Поле текста варианта ответа обязательно для заполнения!';
        ...
        if (!inputAnswersValid) {
          this.state.answers.map((answer) => {
            ...
            if (!answer.inputAnswersValid[answer.answer_id]) {
              this.validateField(
                'text_answer_' + answer.answer_id,
                this.state.inputAnswers[cnt].TextAnswerInput[
                  'text_answer_' + answer.answer_id
                ],
                4,
              );
  ...
  validateForm() {
    this.setState({
      formValid:
        this.state.inputTextOfQuestionValid && this.state.inputFilesTextValid,
                ...
                <div className="mt-5 md:col-span-2 md:mt-0">
                  ...
                  >
                    <div className="inline-flex w-full flex-row">
                      <label
                        htmlFor={`question_text_${count_question + 1}`}
                        className="block text-sm font-medium text-gray-700"
                      >
                        Введите текст вопроса №{count_question + 1}{' '}
              ...
              <button
                ...
                className={`${
                  this.state.formValid
                    ? 'justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2'
