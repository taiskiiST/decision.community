...
const App = () => {
  ...
  const updateChildrenAfterServerResponse = (
    updatedChildren,
    newItemsByKeys,
  ) => {
  ...
  const onPresidiumListChange = async (item, selectedMembers) => {
    ...
    setItemsByKeys((currentItemsByKeys) => {
      ...
      const { currentPresidiumMembers, currentChairman } =
        updatedItemFromServer;
  ...
  const onRevPresidiumListChange = async (item, selectedMembers) => {
    ...
    setItemsByKeys((currentItemsByKeys) => {
      ...
      const { currentRevPresidiumMembers, currentRevChairman } =
        updatedItemFromServer;
  ...
  const getPotentialChairmen = (item) => {
    ...
    return potentialPresidiumMembers.filter((member) => {
      ...
      return currentPresidiumMembers.find(
        (currentPresidiumMemberId) => currentPresidiumMemberId === id,
      );
    });
  };
  ...
  const getPotentialRevChairmen = (item) => {
    ...
    return potentialRevPresidiumMembers.filter((member) => {
      ...
      return currentRevPresidiumMembers.find(
        (currentRevPresidiumMemberId) => currentRevPresidiumMemberId === id,
      );
    });
  };
  ...
  const potentialPresidiumMembers = potentialCommitteeMembers.filter(
    (member) => {
      const { id } = member;

      return currentCommitteeMembers.find(
        (currentCommitteeMemberId) => currentCommitteeMemberId === id,
      );
    },
  );

  const potentialRevPresidiumMembers = potentialRevCommitteeMembers.filter(
    (member) => {
      const { id } = member;

      return currentRevCommitteeMembers.find(
        (currentRevCommitteeMemberId) => currentRevCommitteeMemberId === id,
      );
    },
  );
  ...
  return (
    <div className="min-h-2xl p-2 lg:flex xl:flex 2xl:flex">
      <AddCategoryModal
      ...
      <UpdateThumbModal
        ...
        currentThumbUrl={
          updateThumbModalItem ? updateThumbModalItem.thumbUrl : null
        }
      ...
      <div className="w-full min-w-72 p-1 md:w-72">
        <div className="mb-2">
          <button
            type="button"
            className="inline-flex w-full items-center border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
            id="add-new-category-button"
            onClick={onNewCategoryBtnClick}
          >
            <svg
              className="-ml-1 mr-2 block h-5 w-5"
          ...
          <button
            type="button"
            className="mt-2 inline-flex w-full items-center border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
            id="add-new-category-button"
      ...
      <div className="min-h-full overflow-hidden p-1 md:flex-grow">
        {/*{*/}
