...
class Item extends Model
{
  ...
  {
    return $builder
      ->when($sortBy === ItemsList::SORT_BY_LATEST, function (
        Builder $builder
      ) {
        return $builder->orderBy('updated_at', 'desc');
      })
      ->when($sortBy === ItemsList::SORT_ALPHABETICALLY, function (
        Builder $builder
      ) {
  ...
  public function getAllChildren(): Collection
  {
    ...
    $query = Item::where('parent_id', $this->id)
      ->where('company_id', session('current_company')->id)
      ->unionAll(
        Item::select('items.*')->join('tree', 'tree.id', '=', 'items.parent_id')
      );
    return Item::from('tree')->withRecursiveExpression('tree', $query)->get();
  }
  ...
  public function getAllParents(): Collection
  {
    ...
    $query = Item::where('id', $this->parent_id)
      ->where('company_id', session('current_company')->id)
      ->unionAll(
        Item::select('items.*')->join('tree', 'items.id', '=', 'tree.parent_id')
      );
    //        foreach (){
    //
    //        }
    $search_items = Item::where('parent_id', '=', $this->parent_id)
      ...
      ->where('is_category', 1)
      ->unionAll(
        Item::where('parent_id', '=', $this->id)->where('is_category', 1)
      )
      ->distinct()
      ->get();
  ...
  {
    $revCommitteeMembersByIds = RevCommitteeMember::get()->groupBy(
      'rev_committee_id'
    );

    $revPresidiumMembersByIds = RevPresidiumMember::get()->groupBy(
      'rev_presidium_id'
    );

    $revChairmenByIds = RevChairman::get()->keyBy('rev_chair_id');

    $this->currentRevCommitteeMembers = $revCommitteeMembersByIds->has(
      $this->id
    )
      ? $revCommitteeMembersByIds->get($this->id)->pluck('member_id')->toArray()
      : [];

    $this->currentRevPresidiumMembers = $revPresidiumMembersByIds->has(
      $this->id
    )
  ...
  public function getPeopleThatDidNotVote(
    Collection $peopleThatDidNotVote
  ): Collection {
    return $this->getAllNonCategoryChildren()->filter(function ($child) use (
      $peopleThatDidNotVote
    ) {
      return in_array(
        $child->id,
        $peopleThatDidNotVote->pluck('id')->toArray()
      );
    });
  }
}
