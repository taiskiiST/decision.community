        ...
        return (
                <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                    <table className="min-w-full divide-y divide-gray-200 ">
                        ...
                        <tr>
                            ...
                            {is_admin && <th scope="col" className="relative px-6 py-3">
                                Доступен всем
                            </th>}
                            <th scope="col" className="relative px-6 py-3">
                                Детали
                            </th>
                            {poll_finished && <th scope="col" className="relative px-6 py-3">
                                <span
                                    className="text-green-600">Голосование окончено {poll_full.finished}</span>
                            </th>}
                            { is_admin && !poll_finished && <th scope="col" className="relative px-6 py-3">
                ...
                {questions.map( (question, index) => (
                    <tr className={`bg-white ${index % 2 === 0 ? 'bg-gray-200' : ''}`} key={index}>
                        <td className="px-6 py-4 whitespace-wrap text-wrap text-right text-sm font-medium">
                            {index + 1}
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-wrap text-wrap text-right text-sm font-medium ">
                            {is_admin && <a href={`${poll_finished ? '#' : `/polls/${poll_full.id}/questions/${question.id}/`  }`} key={index}
                               className={`${poll_finished ? 'disabled' : 'text-indigo-600 hover:text-indigo-900'}`} key={index} >
                                Изменить вопрос</a>}
                        </td>

                        {is_admin && <td className="px-6 py-4 whitespace-wrap text-wrap text-right text-sm font-medium">
