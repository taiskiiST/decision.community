  ...
  render() {
    ...
    return (
      <div className="px-1 py-4 sm:px-6 ">
        <label className="text-wrap block px-1 py-4 text-xl font-semibold text-black">
          Начало и окончание
        </label>
        <div className="flex flex-row px-4">
          Текущий статус:&nbsp;
          {poll_full.start && poll_full.finished && (
            <b> закончено в {`${poll_full.finished}`} </b>
          )}
          {poll_full.start && !poll_full.finished && (
            <b> начато в {`${poll_full.start}`} </b>
          )}
          {!poll_full.start && !poll_full.finished && <b> не начато </b>}
        </div>
        <div className="flex flex-row items-center px-4">
          <div>
            <label>Установить дату начала (изменить/удалить дату начала)</label>
          </div>
          <div>
            <input
              type="datetime-local"
              id="date_start"
              defaultValue={poll_full.start ?? Date.now()}
            />
          </div>
          <div>
            <button
              type="button"
              //id="StartButton"
              className="justify-end rounded-md border border-transparent bg-green-600
        ...
        <div className="flex flex-row items-center px-4">
          <div>
            <label>
              Уставноить дату окончания (изменить/удалить дату окончания)
            </label>
          </div>
          <div>
            <input
              type="datetime-local"
              id="date_end"
              defaultValue={poll_full.finished ?? Date.now()}
            />
          </div>
          <div>
            <button
