...
class ChildrenAndParentsInformation extends Controller
{
  ...
  public function submit(Request $request)
  {
    ...
    if ($this->checkInputs($inputs)) {
      ...
      return redirect()
        ->route('children-and-parents-information')
        ->withErrors('Все поля обязательны для заполнения!');
    }
  }
  ...
  public function submitSchool(Request $request)
  {
    ...
    if ($this->checkInputs($inputs)) {
      ...
      return redirect()
        ->route('children-and-parents-information-school')
        ->withErrors('Все поля обязательны для заполнения!');
    }
  }
  ...
  {
    $countChildren = ChildrenInformation::all()
      ->where('school_name', '=', '')
      ->count();
    return view('children-and-parents-info.done', [
  ...
  public function doneSchool()
  {
    $countChildren = ChildrenInformation::all()
      ->where('school_name', '<>', '')
      ->count();
    return view('children-and-parents-info.done-school', [
      'count_children' => $countChildren,
    ]);
  }
  ...
  public function report()
  {
    ...
    foreach ($parents as $key => $parent) {
      ...
      foreach ($parent->children()->get() as $key_child => $child) {
        $information[$key]['child_' . $key_child]['full_name'] =
          $child['full_name'];
        $information[$key]['child_' . $key_child]['sex'] = $child['sex'];
        $time = strtotime($child['date_of_birthday']);
        $newformat = date('d.m.Y', $time);
        $information[$key]['child_' . $key_child][
          'date_of_birthday'
        ] = $newformat;
        $information[$key]['child_' . $key_child]['age'] = $this->ageCalculator(
          $newformat
        );
      }
    }
  ...
  public function reportAge()
  {
    ...
    return view('children-and-parents-info.report-age', [
      ...
      'total_count_children' => ChildrenInformation::all()
        ->where('school_name', '=', '')
        ->count(),
    ]);
  }
  ...
  {
    $inputs = $request->input();
    $parent = ParentInformation::all()
      ->where('phone', 'LIKE', $inputs['phone'])
      ->where('school_information', '1')
      ->first();
    if ($parent) {
      $informations = [];
      $children = ChildrenInformation::all()->where('school_name', '<>', '');
      ...
      return redirect()
        ->route('check-parent')
        ->withErrors('Данный номер телефона не найден!');
    }
  }
  ...
  public function schoolReportBySchool(Request $request)
  {
    ...
    if ($parent) {
      $schoolArr = [];
      $children_group_by_school = ChildrenInformation::all()
        ->where('school_name', '<>', '')
        ->groupBy(['school_name']);
      foreach ($children_group_by_school as $name_school => $school_children) {
        foreach ($school_children as $child) {
          $parent_of_child = $child->parent()->first();
          $ageArr[$name_school . ', ' . $child->school_address][$child->id][] =
            $child->school_time_to;
          $ageArr[$name_school . ', ' . $child->school_address][$child->id][] =
            $child->school_time_from;
          $ageArr[$name_school . ', ' . $child->school_address][$child->id][] =
            $parent_of_child->full_name;
          $ageArr[$name_school . ', ' . $child->school_address][$child->id][] =
            $parent_of_child->relationship;
          $ageArr[$name_school . ', ' . $child->school_address][$child->id][] =
            $parent_of_child->address;
          $ageArr[$name_school . ', ' . $child->school_address][$child->id][] =
            $parent_of_child->phone;
          $ageArr[$name_school . ', ' . $child->school_address][$child->id][] =
            $child->full_name;
          $ageArr[$name_school . ', ' . $child->school_address][$child->id][] =
            $child->sex;
          $time = strtotime($child->date_of_birthday);
          $newformat = date('d.m.Y', $time);
          $ageArr[$name_school . ', ' . $child->school_address][
            $child->id
          ][] = $this->ageCalculator($newformat);
        }
      }
  ...
  public function ageCalculator($newformat)
  {
    ...
    $age = DateTime::createFromFormat('d.m.Y', $newformat, $tz)->diff(
      new DateTime('now', $tz)
    )->y;
    return $age;
  }
