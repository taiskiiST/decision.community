...
@section('content')
    <div class="mt-10 sm:mt-0">
        <div class="p-3 md:col-span-2">
            <label class="px-12 py-4 block text-lg text-black font-semibold">Обновление Должности</label>

            <div class="grid sm:grid-cols-2 lg:grid-cols-2 gap-4">
                <form method="POST" action="{{route('position.update.submit')}}" class="inline-flex flex-col">
                    @csrf
                    <div class="px-1 py-1 bg-white sm:p-6">
                        <div class="col-span-6 sm:col-span-3">
                            ...
                            <input type="text" name="position_name" value="{{$position->position}}" id="position" autocomplete="position" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
                            <input type="text" name="position_id" value="{{$position->id}}" class="hidden">
                        </div>
                        <button type="submit"
